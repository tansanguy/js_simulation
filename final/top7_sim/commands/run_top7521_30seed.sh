#!/usr/bin/env bash
# run_top7521_30seed.sh
# top7521 확정 후보 7개 × 30seed baseline/smart 시뮬레이션 실행 스크립트
#
# 후보 목록 (top7521):
#   LINK_239754 / NODE_5831 / NODE_5846
#   LINK_10218  / LINK_120139
#   NODE_10335  / NODE_10376
#
# 사용법:
...
#   bash final/top7_sim/commands/run_top7521_30seed.sh --jobs=4 --seeds 1-10 --dry-run
#   bash final/top7_sim/commands/run_top7521_30seed.sh --skip-if-done
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --jobs)
      shift
      if [[ $# -eq 0 || "$1" == --* ]]; then
        echo "--jobs requires a value" >&2; exit 1
      fi
      JOBS="$1"; shift ;;
    --jobs=*)     JOBS="${1#*=}";        shift ;;
    --seeds)
      shift
      if [[ $# -eq 0 || "$1" == --* ]]; then
        echo "--seeds requires a value" >&2; exit 1
      fi
      SEEDS_SPEC="$1"; shift ;;
...
if [[ ${#POSITIONAL[@]} -gt 0 ]]; then
  echo "Unknown option(s): ${POSITIONAL[*]}" >&2; exit 1
fi
...
run_sampled() {
  ...
  echo "[top7521][start] seed=$seed role=$manifest_row_role crosswalk=$manifest_crosswalk_id"
...
if [[ ! -f "$BASELINE_CSV" ]]; then
  echo "baseline CSV 없음: $BASELINE_CSV" >&2; exit 1
fi
...
if [[ "$DRY_RUN" == true ]]; then
  echo "[top7521][dry-run] jobs=${JOBS} seeds=${SEEDS[*]} skip_if_done=${SKIP_IF_DONE}"
  echo "[top7521][dry-run] baseline_runs=${#SEEDS[@]}"
  echo "[top7521][dry-run] smart_ids=${SMART_IDS[*]}"
  echo "[top7521][dry-run] smart_runs=$((${#SMART_IDS[@]} * ${#SEEDS[@]}))"
  echo "[top7521][dry-run] total_runs=$((${#SEEDS[@]} + ${#SMART_IDS[@]} * ${#SEEDS[@]}))"
  exit 0
fi

# ── baseline seeds ─────────────────────────────────────────────────────────────
echo "[top7521] baseline seeds=${SEEDS[*]} 시작 (jobs=${JOBS}, sim=1800s warmup=300s SSM=on)"
for seed in "${SEEDS[@]}"; do
  ...
  run_sampled_async "$BASELINE_CSV" "$out_dir" "$log_file" \
    "$seed" "baseline_placeholder" "BASELINE_TOP7521"
done
wait_for_all_jobs
echo "[top7521] baseline 완료"

# ── smart 7개 × seeds ─────────────────────────────────────────────────────────
echo "[top7521] smart seeds=${SEEDS[*]} × ${#SMART_IDS[@]}후보 시작 (jobs=${JOBS})"
